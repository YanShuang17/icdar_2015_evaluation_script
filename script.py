...
import importlib
import os
def read_floder(floder_path):
    pairs = []
    for file in os.listdir(floder_path):
        keyName = file.split('_')[-1].split('.')[0]
        with open(os.path.join(floder_path, file)) as f:
            content = f.read()
        pairs.append([keyName, content])
    return dict(pairs)
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    gt = read_floder(gtFilePath)
    subm = read_floder(submFilePath)
...
if __name__=='__main__':
    ...
    rrc_evaluation_funcs.main_evaluation(None,default_evaluation_params, evaluate_method)
